        ...
        if (!v.hasData) {
            ...
            if (config.notifyOpsOnMismatch && ((ServerWorld) player.getEntityWorld()).getServer() != null && !hideOwner) {
                ...
                for (ServerPlayerEntity op : srv.getPlayerManager().getPlayerList()) {
                    boolean isOpPlayer = false;
                    String opName = op.getNameForScoreboard();
                    for (String o : srv.getPlayerManager().getOpNames()) {
                        if (o.equalsIgnoreCase(opName)) { isOpPlayer = true; break; }
                    }
                    if (op != player && isOpPlayer) {
