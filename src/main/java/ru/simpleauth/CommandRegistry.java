...
public class CommandRegistry {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
                dispatcher.register(CommandManager.literal(name)
                        ...
                        .executes(ctx -> {
                            ServerPlayerEntity player = ctx.getSource().getPlayer();
                            if (player == null) return 0;
                            boolean logging = ctx.getSource().getServer().getOverworld().getGameRules()
                                    .getValue(GameRules.LOG_ADMIN_COMMANDS);
                            player.sendMessage(Text.literal(String.format(
    ...
    private static int setQuiet(SessionManager manager,
        ...
        try {
            boolean v = !quiet;
            server.getOverworld().getGameRules().setValue(GameRules.LOG_ADMIN_COMMANDS, v, server);
            server.getOverworld().getGameRules().setValue(GameRules.SEND_COMMAND_FEEDBACK, v, server);
        } catch (Exception e) {
            return 0;
        }
     ...
     */
    /**
     * Проверка прав для админ-команд. В 1.21.11 переписали API прав
     * (hasPermissionLevel убрали), поэтому проверяем через список
     * операторов сервера — стабильный способ. Консоль/командный блок
     * (не игрок) считаем полноправными.
     */
    private static boolean isOp(net.minecraft.server.command.ServerCommandSource source) {
        net.minecraft.entity.Entity e = source.getEntity();
        if (e instanceof ServerPlayerEntity p) {
            String name = p.getNameForScoreboard();
            for (String op : source.getServer().getPlayerManager().getOpNames()) {
    ...
    private static int runDeathTest(SessionManager manager, CommandContext<ServerCommandSource> ctx) {
        ...
        ArmorStandEntity dummy = EntityType.ARMOR_STAND.create(world, net.minecraft.entity.SpawnReason.COMMAND);
        if (dummy == null) {
