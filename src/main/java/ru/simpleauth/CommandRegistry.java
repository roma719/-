...
public class CommandRegistry {
    ...
    private static int setQuiet(SessionManager manager,
        ...
        try {
            server.getOverworld().getGameRules().get(GameRules.LOG_ADMIN_COMMANDS).set(!quiet, server);
            server.getOverworld().getGameRules().get(GameRules.SEND_COMMAND_FEEDBACK).set(!quiet, server);
        } catch (Exception e) {
            return 0;
        }
